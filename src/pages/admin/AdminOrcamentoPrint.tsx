...
const STATUS_COLOR: Record<string, string> = {
  rascunho: '#6b7280', enviado: '#2563eb', aprovado: '#16a34a',
  recusado: '#dc2626', executado: '#7c3aed', faturado: '#d97706',
}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div style={{
      ...
      borderLeft: `3px solid ${BRAND.yellow}`, paddingLeft: 6,
    }}>
      {children}
    </div>
  )
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: BRAND.navy }}>
        <p style={{ color: BRAND.yellow, fontFamily: 'sans-serif' }}>Carregando orçamento...</p>
      </div>
    )
  }
  if (!orc) return null
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)', marginTop: 8, lineHeight: 1.9 }}>
              <div>Emissão: <strong style={{ color: BRAND.white }}>{formatDate(orc.data_emissao)}</strong></div>
              {orc.data_validade && (
                <div>Validade: <strong style={{ color: BRAND.white }}>{formatDate(orc.data_validade)}</strong></div>
              )}
            </div>
            <div style={{ marginTop: 10 }}>
              <span style={{
                background: statusColor, color: BRAND.white,
        ...
        <div style={{ padding: '32px 40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 28 }}>
            <InfoBox>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: BRAND.gray900, marginBottom: 4 }}>{orc.cliente_nome}</div>
              {orc.cliente_cpf_cnpj && <div style={{ fontSize: 12, color: BRAND.gray700 }}>CPF/CNPJ: {orc.cliente_cpf_cnpj}</div>}
              {orc.cliente_telefone && <div style={{ fontSize: 12, color: BRAND.gray700 }}>Tel: {orc.cliente_telefone}</div>}
              {orc.cliente_email && <div style={{ fontSize: 12, color: BRAND.gray700 }}>{orc.cliente_email}</div>}
            </InfoBox>
            ...
            <InfoBox>
              ...
              {(orc.local_servico || orc.cliente_endereco) ? (
                <>
                  <div style={{ fontSize: 12, color: BRAND.gray700, marginBottom: 2 }}>
                    {orc.local_servico || orc.cliente_endereco}
                  </div>
                  {orc.cliente_cidade && (
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: BRAND.blue, marginBottom: 6 }}>{orc.tipo_servico}</div>
            {orc.descricao_servico && (
              <div style={{ fontSize: 12, color: BRAND.gray700, lineHeight: 1.6, background: BRAND.gray50, border: `1px solid ${BRAND.gray100}`, borderRadius: 4, padding: '10px 14px' }}>
                {orc.descricao_servico}
              </div>
            )}
          </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr style={{ background: BRAND.navy, color: BRAND.white }}>
                    <th style={{ padding: '9px 12px', textAlign: 'left', fontWeight: 600 }}>Descrição</th>
                    <th style={{ padding: '9px 12px', textAlign: 'center', fontWeight: 600, width: 60 }}>Qtd</th>
                    <th style={{ padding: '9px 12px', textAlign: 'center', fontWeight: 600, width: 50 }}>Un.</th>
                    <th style={{ padding: '9px 12px', textAlign: 'right', fontWeight: 600, width: 110 }}>Valor Unit.</th>
                    <th style={{ padding: '9px 12px', textAlign: 'right', fontWeight: 600, width: 110, borderLeft: `2px solid ${BRAND.yellow}` }}>Total</th>
                  </tr>
                </thead>
                <tbody>
                  {itens.map((item, idx) => (
                    <tr key={item.id} style={{ background: idx % 2 === 0 ? BRAND.white : BRAND.gray50 }}>
                      <td style={{ padding: '8px 12px', borderBottom: `1px solid ${BRAND.gray100}`, color: BRAND.gray900 }}>{item.descricao}</td>
                      <td style={{ padding: '8px 12px', borderBottom: `1px solid ${BRAND.gray100}`, textAlign: 'center', color: BRAND.gray700 }}>
                        {Number(item.quantidade) % 1 === 0 ? Number(item.quantidade) : Number(item.quantidade).toFixed(2)}
                      </td>
                      <td style={{ padding: '8px 12px', borderBottom: `1px solid ${BRAND.gray100}`, textAlign: 'center', color: BRAND.gray700 }}>{item.unidade}</td>
                      <td style={{ padding: '8px 12px', borderBottom: `1px solid ${BRAND.gray100}`, textAlign: 'right', color: BRAND.gray700 }}>{formatBRL(item.valor_unitario)}</td>
                      <td style={{ padding: '8px 12px', borderBottom: `1px solid ${BRAND.gray100}`, textAlign: 'right', fontWeight: 700, color: BRAND.navy, borderLeft: `2px solid ${BRAND.yellow}` }}>{formatBRL(item.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 28 }}>
            <div style={{ width: 280 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: `1px solid ${BRAND.gray100}`, fontSize: 13 }}>
                <span style={{ color: BRAND.gray500 }}>Subtotal</span>
                <span style={{ color: BRAND.gray900 }}>{formatBRL(orc.subtotal)}</span>
              </div>
              {orc.desconto > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: `1px solid ${BRAND.gray100}`, fontSize: 13 }}>
              ...
              <div style={{
                ...
                background: BRAND.navy, borderRadius: 6,
              }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: BRAND.yellow, textTransform: 'uppercase', letterSpacing: 1 }}>Total</span>
                <span style={{ fontSize: 20, fontWeight: 900, color: BRAND.white }}>{formatBRL(orc.total)}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: orc.prazo_execucao ? '1fr 1fr' : '1fr', gap: 16, marginBottom: 24 }}>
            <InfoBox>
              ...
              <div style={{ fontSize: 13, color: BRAND.gray900, fontWeight: 600 }}>{orc.condicoes_pagamento}</div>
            </InfoBox>
            {orc.prazo_execucao && (
              <InfoBox>
                <SectionLabel>Prazo de Execução</SectionLabel>
                <div style={{ fontSize: 13, color: BRAND.gray900, fontWeight: 600 }}>{orc.prazo_execucao}</div>
              </InfoBox>
            )}
          </div>
          ...
          {orc.observacoes && (
            <div style={{
              ...
              <div style={{ fontSize: 12, color: BRAND.gray700, lineHeight: 1.7, whiteSpace: 'pre-line' }}>{orc.observacoes}</div>
            </div>
          )}
          ...
          {fotos.length > 0 && (
            <div style={{ marginBottom: 32 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginTop: 10 }}>
                {fotos.map(foto => (
                  <div key={foto.id} style={{ borderRadius: 6, overflow: 'hidden', border: `1px solid ${BRAND.gray100}` }}>
                    <img
                      src={foto.url}
                    ...
                    />
                    {foto.legenda && (
                      <div style={{ padding: '4px 8px', fontSize: 10, color: BRAND.gray500, background: BRAND.gray50 }}>
                        {foto.legenda}
                      </div>
                    )}
                  </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, marginTop: 56, marginBottom: 16 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ borderTop: `2px solid ${BRAND.navy}`, paddingTop: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: BRAND.navy }}>{config.nome_empresa}</div>
                <div style={{ fontSize: 10, color: BRAND.gray500, marginTop: 2 }}>Responsável Técnico</div>
              </div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ borderTop: `2px solid ${BRAND.navy}`, paddingTop: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: BRAND.navy }}>{orc.cliente_nome}</div>
