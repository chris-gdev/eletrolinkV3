...
function emptyForm(): Omit<
  OrcamentoFormal,
  "id" | "numero" | "created_at" | "updated_at" | "itens"
> {
  const emissao = today();
  return {
    cliente_nome: "",
    ...
    data_validade: addDays(emissao, 30),
    subtotal: 0,
    desconto: 0,
    total: 0,
...
export default function AdminOrcamentoForm() {
  ...
  useEffect(() => {
    if (!isEdit || !id) return;
    async function load() {
      const [{ data: orc, error }, { data: fotos }] = await Promise.all([
        supabase
  ...
  async function uploadPendingFotos(orcId: string) {
    ...
    for (const { file } of pendingFiles) {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("orcamento-fotos").getPublicUrl(path);
  ...
  function updateItem(
    key: string,
    field: keyof Omit<ItemLocal, "_key">,
    raw: string | number,
  ) {
    setItens((prev) =>
      prev.map((i) => {
        ...
        if (field === "quantidade" || field === "valor_unitario") {
          updated.total =
            Number(updated.quantidade) * Number(updated.valor_unitario);
        }
        return updated;
      }),
    );
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <button
            ...
            className="flex items-center gap-1.5 text-gray-500 hover:text-white transition-colors font-body text-sm mb-2"
          >
            <ArrowLeft size={14} /> Voltar
          </button>
          <h1 className="font-display font-bold text-2xl lg:text-3xl text-white uppercase tracking-wider">
            {isEdit
              ? `Orçamento #${String(numero).padStart(4, "0")}`
              : "Novo Orçamento"}
          </h1>
        </div>
        <div className="flex gap-3 flex-wrap">
          {isEdit && (
            <button
              onClick={() =>
                window.open(`/admin/orcamentos/${id}/imprimir`, "_blank")
              }
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      status: e.target.value as StatusOrcamentoFormal,
                    }))
                  }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, data_emissao: e.target.value }))
                }
                className="input-dark"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, data_validade: e.target.value }))
                }
                className="input-dark"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, prazo_execucao: e.target.value }))
                }
        ...
        <div className="card-dark p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) =>
                    setForm((p) => ({ ...p, tipo_servico: e.target.value }))
                  }
                  className="input-dark appearance-none pr-8"
                >
            ...
            {f(
              "Local do Serviço (se diferente do endereço do cliente)",
              "local_servico",
              "text",
              "Endereço da obra / local de instalação",
            )}
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, descricao_servico: e.target.value }))
                }
        ...
        <div className="card-dark p-6">
          ...
          <div className="hidden lg:grid grid-cols-[1fr_80px_80px_120px_120px_36px] gap-2 mb-2">
            {[
              ...
              <div
                key={h}
                className="text-gray-500 font-body text-xs uppercase tracking-wider"
              >
                {h}
              </div>
            ))}
          ...
          <div className="space-y-2">
            {itens.map((item, idx) => (
              <div
                key={item._key}
                className="grid grid-cols-1 lg:grid-cols-[1fr_80px_80px_120px_120px_36px] gap-2 items-center"
              >
                <input
                  type="text"
                  value={item.descricao}
                  onChange={(e) =>
                    updateItem(item._key, "descricao", e.target.value)
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateItem(item._key, "quantidade", e.target.value)
                  }
                ...
                <div className="relative">
                  <select
                    value={item.unidade}
                    onChange={(e) =>
                      updateItem(item._key, "unidade", e.target.value)
                    }
            ...
            <div className="w-full max-w-xs space-y-2">
              ...
              <div className="flex items-center justify-between gap-4 text-sm font-body">
                ...
                <input
                  ...
                  value={form.desconto}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, desconto: Number(e.target.value) }))
                  }
        ...
        <div className="card-dark p-6">
          ...
          {totalFotos === 0 && (
            ...
            >
              <Image
                size={32}
                className="mx-auto text-dark-400 group-hover:text-primary-500/50 mb-3 transition-colors"
              />
              <p className="text-gray-500 font-body text-sm">
                Clique ou arraste imagens aqui
              </p>
              <p className="text-gray-600 font-body text-xs mt-1">
                JPG, PNG, WEBP — múltiplas fotos permitidas
              </p>
            </div>
          )}
          ...
          {totalFotos > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              ...
              {fotosExistentes.map((foto) => (
                <div
                  key={foto.id}
                  className="relative group aspect-square rounded-lg overflow-hidden border border-dark-500"
                >
                  <img
                    src={foto.url}
                    alt={foto.nome}
              ...
              {pendingFiles.map((pf) => (
                <div
                  key={pf._key}
                  className="relative group aspect-square rounded-lg overflow-hidden border border-primary-500/30"
                >
                  <img
                    src={pf.preview}
                    alt={pf.file.name}
                  ...
                  <div className="absolute top-1.5 left-1.5">
                    <span className="bg-primary-500 text-white text-xs px-1.5 py-0.5 rounded font-body">
                      Novo
                    </span>
                  </div>
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                    <button
                      onClick={() => removePending(pf._key)}
                      className="bg-red-500 hover:bg-red-600 text-white p-1.5 rounded-full transition-colors"
                    >
                      <X size={14} />
                    </button>
              ...
              >
                <Plus
                  size={20}
                  className="text-dark-400 group-hover:text-primary-500/50 transition-colors mb-1"
                />
          ...
          {pendingFiles.length > 0 && (
            <p className="mt-3 text-xs font-body text-gray-500">
              {pendingFiles.length} foto{pendingFiles.length > 1 ? "s" : ""}{" "}
              aguardando — serão enviadas ao salvar o orçamento.
            </p>
          )}
        </div>
        ...
        <div className="card-dark p-6">
          ...
          <p className="text-gray-500 font-body text-xs mb-3">
            Estas notas não aparecem no PDF impresso.
          </p>
          <textarea
            value={form.notas_internas}
            onChange={(e) =>
              setForm((p) => ({ ...p, notas_internas: e.target.value }))
            }
